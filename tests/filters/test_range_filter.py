...
class TestRangeFilterBuildSelect(AssertsCompiledSQL):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([1, 10], "item.area >= 1 AND item.area <= 10"),
            ((10, 1), "item.area >= 10 AND item.area <= 1"),
            ((None, 1), "item.area <= 1"),
            ((1, None), "item.area >= 1"),
        ],
    )
    def test_filtering(self, value: Any, expected_filtering: str) -> None:
        ...
        self.assert_compile(
            stmt, f"SELECT item.id FROM item WHERE {expected_filtering}", literal_binds=True
        )

    @pytest.mark.parametrize(
        "value, expected_filtering",
        [
            ([1, 10], "item.area < 1 OR item.area > 10"),
            ((None, 1), "item.area > 1"),
            ((1, None), "item.area < 1"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([1, 10], "item.area <= 1 OR item.area > 10"),
            ((1, None), "item.area <= 1"),  # param > left_val --> param <= left_val
            ((None, 1), "item.area > 1"),  # param <= right_val --> param > right_val
        ],
    )
